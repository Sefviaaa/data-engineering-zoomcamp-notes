...
def download_upload_tripdata(
    ...
    with dag:
        ...
        create_native_table_task = BigQueryInsertJobOperator(
            task_id=f"create_native_table_{dataset}",
            configuration={
                "query": {
                    "query": f"""
                        CREATE OR REPLACE TABLE
                        `{PROJECT_ID}.nytaxi.{dataset}_native_{{{{ execution_date.year }}}}` AS
                        SELECT *
                        FROM `{PROJECT_ID}.nytaxi.{dataset}_tripdata_external_{{{{ execution_date.year }}}}`;
                    """,
                    "useLegacySql": False,
                }
            },
        ...
        create_partitioned_table_task = BigQueryInsertJobOperator(
            task_id=f"create_partitioned_table_{dataset}",
            configuration={
                "query": {
                    "query": f"""
                        CREATE OR REPLACE TABLE
                        `{PROJECT_ID}.nytaxi.{dataset}_partitioned_{{{{ execution_date.year }}}}`
                        PARTITION BY DATE(lpep_pickup_datetime)
                        CLUSTER BY PULocationID AS
                        SELECT *
                        FROM `{PROJECT_ID}.nytaxi.{dataset}_tripdata_external_{{{{ execution_date.year }}}}`
                        WHERE lpep_pickup_datetime >= TIMESTAMP('{{{{ execution_date.year }}}}-01-01')
                        AND lpep_pickup_datetime <  TIMESTAMP('{{{{ execution_date.year + 1 }}}}-01-01');
                    """,
                    "useLegacySql": False,
                }
            },
        ...
        download_task >> upload_task >> create_external_table_task >> create_native_table_task >> create_partitioned_table_task
